...
ARCH="${ARCH:-amd64}"
DOCKER_DOWNLOAD_URL="${DOCKER_DOWNLOAD_URL:-https://download.docker.com/linux/static/stable/x86_64/docker-19.03.3.tgz}"
...
if [[ -z ${NAME:-} ]]; then
  echo >&2 "USAGE: $0 POD_NAME [-k KUBECONFIG] [-n POD_NAMESPACE] [-c CONTAINER_NAME]"
  exit 1
fi

# Internal variables
KUBECTL="kubectl"
[ -z "${KUBECONFIG}" ] || KUBECTL="${KUBECTL} --kubeconfig=${KUBECONFIG}"
...
DOCKERCMD="/opt/docker/docker -H unix:///mnt/docker.sock"
...
INSTALLCMD="${INSTALLCMD} wget -qO /docker.tgz ${DOCKER_DOWNLOAD_URL}"
# Extract docker client
INSTALLCMD="${INSTALLCMD} && mkdir -p /opt/"
INSTALLCMD="${INSTALLCMD} && tar zxvf /docker.tgz -C /opt/"
#INSTALLCMD="${INSTALLCMD} && chmod u+x /opt/docker/docker"
